// JavaScript solution for predicting robot motion and calculating safety factor
...
function simulateRobots(input, time) {
  return input.map((robot) => {
    const [x, y] = robot.position;
    const [vx, vy] = robot.velocity;
    // Compute new position after `time` seconds, applying wrapping
    const newX = (x + vx * time) % GRID_WIDTH;
    const newY = (y + vy * time) % GRID_HEIGHT;
    // Handle negative wrapping
    return {
      x: (newX + GRID_WIDTH) % GRID_WIDTH,
      y: (newY + GRID_HEIGHT) % GRID_HEIGHT,
    };
  });
}

// Count robots in each quadrant
function calculateSafetyFactor(robots) {
  ...
  robots.forEach(({ x, y }) => {
    if (x === midX || y === midY) return; // Exclude middle line
  ...
  return quadrants.reduce((acc, val) => acc * val, 1); // Multiply counts
}
...
const inputActual = `p=43,30 v=73,-79
p=20,21 v=-72,96
p=24,57 v=-95,97
p=29,86 v=-19,83
p=30,80 v=-91,-16
...
function parseInput(input) {
  return input.split("\n").map((line) => {
    ...
    console.log("position, velocity", positionPart, velocityPart);
    const position = positionPart
      .slice(2) // Remove the "p=" prefix
      .split(",")
      .map(Number);
    const velocity = velocityPart
      .slice(2) // Remove the "v=" prefix
      .split(",")
      .map(Number);
...
// Main execution
const finalPositions = simulateRobots(parseInput(inputActual), TIME);
const safetyFactor = calculateSafetyFactor(finalPositions);
