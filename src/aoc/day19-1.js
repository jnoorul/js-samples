...
const towelPatterns = inputPatterns.split(",").map((row) => row.trim());
const designs = inputDesigns.split("\n");

// Function to check if a design can be formed
function canFormDesign(towelPatterns, design) {
  ...
  for (let i = 1; i <= design.length; i++) {
    for (const pattern of towelPatterns) {
      ...
      if (
        i >= patternLength &&
        design.slice(i - patternLength, i) === pattern &&
        dp[i - patternLength]
      ) {
        dp[i] = true;
        break;
...
// Determine which designs are possible
const results = designs.map((design) => ({
  design,
  possible: canFormDesign(towelPatterns, design),
}));

// Output the results
console.log("Results:");
results.forEach(({ design, possible }) => {
  console.log(`${design}: ${possible ? "Possible" : "Impossible"}`);
});

// Count the number of possible designs
const possibleCount = results.filter((result) => result.possible).length;
console.log(`\nNumber of possible designs: ${possibleCount}`);
